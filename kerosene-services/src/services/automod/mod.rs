...
mod old {
    ...
    impl AutomodRuleset {
                    ...
                    AutomodTrigger::TextRegex { deny, allow } => {
                        ...
                        if !allow.is_empty() {
                                ...
                                .build()
                            {
                                ...
                                Err(e) => {
                                    warn!("Invalid regex allow set in rule {}: {}", rule.id, e)
                                }
                            }
                        }
                    }
                    AutomodTrigger::TextKeywords {
                        deny: keywords,
                        allow,
                    } => {
                        for p in keywords {
                        ...
                        if !allow.is_empty() {
                            match regex::RegexSetBuilder::new(
                                allow.iter().map(|s| regex::escape(s)),
                            )
                            .case_insensitive(true)
                            .build()
                            {
                                ...
                                Err(e) => {
                                    warn!("Invalid keyword allow set in rule {}: {}", rule.id, e)
                                }
                            }
                        }
                    }
        ...
        fn scan_text(
            ...
            let mut result = AutomodScan::default();
            ...
            for action in &final_actions.inner {
                match action {
                    AutomodAction::SendAlert {
                        ...
                        let alert_message_fut = async move {
                            let mut txn = self.globals.begin().await?;
                            let msg_id = txn.message_create(message_create).await?;
                            txn.commit().await?;
                            ...
                            let message = self
                                ...
                                .begin_read()
                                .await?
                                .message_get(*alert_channel_id, msg_id)
                                .await?;
                        ...
                        if let Err(e) = alert_message_fut.await {
                            error!(
