...
pub struct SfuShard {
    ...
    command_rx: UnboundedReceiver<ShardMessage>,
...
pub enum ShardMessage {
    Packet { source: SocketAddr, data: Bytes },
    Backend(SfuCommand),
    Backbone(BackboneEvent),
}
...
impl Sfu {
    ...
    pub async fn serve(mut self) {
        ...
        // start workers
        // let (command_txs, shards) = self.spawn_shards();
    ...
    pub async fn handle_packet(&mut self, bytes: Bytes) {
        // extract ufrag
        todo!("send to all shards")

        // extract_stun_ufrag(&bytes);
...
impl SfuShard {
    ...
    fn handle_command(&mut self, command: SfuCommand) {
        match command {
            ...
            SfuCommand::Signalling { peer_id, inner } => todo!("forward to peer_id"),
