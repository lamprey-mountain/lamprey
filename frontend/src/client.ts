...
function reconcile(base: Data, msg: any): Data {
  if (msg.type === "ready") {
    ...
    return { ...base, threads: { ...base.threads, [msg.thread.thread_id]: new Thread(msg.thread, []) }};
    ...
    return {
      ...
      threads: {
        ...
        [message.thread_id]: {
          ...thread,
          messages: [ ...thread.messages.filter(i => i !== message.nonce), message.message_id ],
        },
      },
      messages: {
        ...base.messages,
...
export class Client {
  ...
  async http(method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE", url: string, body?: any) {
