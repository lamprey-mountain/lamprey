...
export const ChatMain = (props: ChatProps) => {
	...
	async function handleSubmit({ text }: { text: string }) {
		...
		await props.client.http(
			"POST",
			`/api/v1/rooms/${props.roomId}/threads/${props.threadId}/messages`,
			{
				content: text,
				nonce: nonce,
			},
...
export const ChatNav = (props: ChatProps) => {
	return (
		<nav style="width:180px;background:var(--color-bg3)">
			<ul>
				<For each={Object.values(props.data.rooms)}>
					{(room) => (
						<li style={`color:${props.roomId === room.room_id ? "red" : ""}`}>
							{room.name}
							<Show when={props.roomId === room.room_id}>
								<ul style="margin-left: 10px">
									<For
										each={Object.values(props.data.threads).filter((i) =>
											i.data.room_id === props.roomId
										)}
									>
										{(thread) => (
											<li
												style={`color:${
													props.threadId === thread.data.thread_id ? "blue" : ""
												}`}
											>
												thread {thread.data.name}
											</li>
										)}
									</For>
								</ul>
