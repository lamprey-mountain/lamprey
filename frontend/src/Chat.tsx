...
import type { Message, User } from "sdk";
	...
	return (
		<Show when={user()}>
			<li
				onMouseDown={(e) => {
					e.preventDefault();
					props.onSelect(props.user_id, user()!.name);
				}}
			>
				<b>{user()!.name}</b>
				<span class="dim">({props.user_id})</span>
			</li>
		</Show>
	);
};

const ThreadSuggestion = (props: {
	thread: ThreadT;
	onSelect: (id: string, name: string) => void;
}) => {
	return (
		<li
			onMouseDown={(e) => {
				e.preventDefault();
				props.onSelect(props.thread.id, props.thread.name);
			}}
		>
			<b>{props.thread.name}</b>
			<span class="dim">({props.thread.id})</span>
		</li>
	);
};

const SearchFiltersDropdown = (props: {
	thread: ThreadT;
	query: string;
	onSelect: (value: string, isSuggestion: boolean) => void;
}) => {
	const api = useApi();
	...
	const currentFilter = createMemo(() => {
		...
		if (lastPart.startsWith("author:")) {
			return { type: "author", value: lastPart.substring(7).toLowerCase() };
		}
		if (
			props.thread.type !== "Dm" && props.thread.type !== "Gdm" &&
			lastPart.startsWith("thread:")
		) {
			return { type: "thread", value: lastPart.substring(7).toLowerCase() };
		}
		return null;
	});
	...
	const authorSuggestions = createMemo(() => {
		...
		return users
			.filter(
				(u) =>
					u.name.toLowerCase().includes(filter.value) ||
					u.id.toLowerCase().includes(filter.value),
			)
			.map((u) => u.id)
			.slice(0, 10);
	});
	...
	const threadSuggestions = createMemo(() => {
		const filter = currentFilter();
		if (filter?.type !== "thread") return [];
		const threads = roomThreads()?.items ?? [];
		if (!filter.value) return threads.slice(0, 10);
		return threads
			.filter(
				(t) =>
					t.name.toLowerCase().includes(filter.value) ||
					t.id.toLowerCase().includes(filter.value),
			)
			.slice(0, 10);
	});

	const onSuggestionSelect = (type: "author" | "thread", id: string) => {
		const parts = props.query.trim().split(/\s+/);
		parts[parts.length - 1] = `${type}:${id}`;
		props.onSelect(parts.join(" ") + " ", true);
	};

	const addFilter = (filter: string) => (e: MouseEvent) => {
		e.preventDefault();
		props.onSelect(filter, false);
	};

	return (
		<div class="search-filters-dropdown">
			<Show when={!currentFilter()}>
				<ul>
					<li onMouseDown={addFilter("author:")}>
						<b>author:</b>
						<span class="dim">find messages by user</span>
					</li>
					<Show
						when={props.thread.type !== "Dm" && props.thread.type !== "Gdm"}
					>
						<li onMouseDown={addFilter("thread:")}>
							<b>thread:</b>
							<span class="dim">search in a specific thread</span>
						</li>
					</Show>
					<li onMouseDown={addFilter("before:")}>
						<b>before:</b>
						<span class="dim">YYYY-MM-DD</span>
					</li>
					<li onMouseDown={addFilter("after:")}>
						<b>after:</b>
						<span class="dim">YYYY-MM-DD</span>
					</li>
				</ul>
			</Show>
			<Show when={currentFilter()?.type === "author"}>
				<ul>
					<For each={authorSuggestions()}>
						{(user_id) => (
							<AuthorSuggestion
								user_id={user_id}
								onSelect={(id) => onSuggestionSelect("author", id)}
							/>
						)}
					</For>
				</ul>
			</Show>
			<Show when={currentFilter()?.type === "thread"}>
				<ul>
					<For each={threadSuggestions()}>
						{(thread) => (
							<ThreadSuggestion
								thread={thread}
								onSelect={(id) => onSuggestionSelect("thread", id)}
							/>
						)}
					</For>
				</ul>
...
export const ChatSearch = (props: { thread: ThreadT }) => {
	...
	createEffect(() => {
		setQuery(ctx.thread_search.get(props.thread.id)?.query ?? "");
	});
	...
	return (
		<div
			...
			onFocusIn={() => setShowFilters(true)}
			onFocusOut={(e) => {
				if (!searchContainerRef?.contains(e.relatedTarget as Node)) {
					setShowFilters(false);
				}
			}}
		>
			...
			<Show when={showFilters()}>
				<SearchFiltersDropdown
					thread={props.thread}
					query={query()}
					onSelect={(value, isSuggestion) => {
						if (isSuggestion) {
							setQuery(value);
						} else {
							setQuery((q) => (q ? q.trim() + " " : "") + value);
						}
						searchContainerRef?.querySelector("input")?.focus();
					}}
				/>
			</Show>
		</div>
