...
export const RoomHome = (props: { room: RoomT }) => {
  ...
  const [threads, { refetch: fetchThreads }] = createResource<Pagination<ThreadT>, string>(() => props.room.id, async (room_id, { value }) => {
	...
	return (
		<div class="flex-1 bg-bg2 text-fg2 p-4">
	    	...
	    	<For each={threads()?.items}>{thread => (
	      	<li>
	      	<article class="contain-content bg-bg3 my-[8px] border-[1px] border-sep [contain:content]">
		      	<header
		      	...
		      	>
			        <div class="flex items-center gap-[8px] leading-none">
			          ...
			          <div class="text-fg2">created time</div>
			        </div>
			        <div class="self-start mt-[8px] text-fg2 cursor-pointer hover:text-fg1 hover:underline" onClick={() => ctx.dispatch({ do: "setView", to: { view: "thread", room: props.room, thread }})}>
			          message count &bull; last msg time
		          	<br />
			          thread description?
			        </div>
		      	</header>
