...
export class Invites {
	...
	_cachedChannelListings = new Map<string, Listing<Invite>>();
	...
	list_room(room_id_signal: () => string): Resource<Pagination<Invite>> {
	...
	list_channel(channel_id_signal: () => string): Resource<Pagination<Invite>> {
		const paginate = async (pagination?: Pagination<Invite>) => {
			if (pagination && !pagination.has_more) return pagination;
		...
		this._cachedChannelListings.set(channel_id, l2);
