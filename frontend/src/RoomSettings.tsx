...
function AuditLog(props: VoidProps<{ room: RoomT }>) {
	const api = useApi();
		...
		<>
			...
			<Show when={log()}>
				<ul>
					<For each={log()!.items}>
						{(entry) => {
							const member = api.room_members.fetch(() => props.room.id, () => entry.user_id)
							const user = api.users.fetch(() => entry.user_id)
							const m = member.error ? { membership: null } : member();
							const name = (m?.membership === "Join" ? m.override_name : null) || user()?.name;
							return(
							<li data-id={entry.id}>
								<details>
