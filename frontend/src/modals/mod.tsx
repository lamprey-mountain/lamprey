...
export function getModal(modal: ModalType) {
	...
	if (isChannelTopic(modal)) {
		return <ModalChannelTopic channel_id={modal.channel_id} />;
	}
	if (isLink(modal)) {
		return <ModalLink editor={modal.editor} />;
	}
	if (isKick(modal)) {
		return (
			<ModalKick api={api2} room_id={modal.room_id} user_id={modal.user_id} />
		);
	}
	if (isBan(modal)) {
		return (
			<ModalBan api={api2} room_id={modal.room_id} user_id={modal.user_id} />
		);
	}
	if (isTimeout(modal)) {
		return (
...
const ModalAlert = (props: { text: string }) => {
	...
	return (
		...
		>
			<p>{props.text}</p>
			<div class="bottom">
				<button type="button" class="button" ref={btn} onClick={modalCtl.close}>
					okay!
				</button>
			</div>
		</Modal>
