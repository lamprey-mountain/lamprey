export function createKeybinds(binds: Record<string, (e: KeyboardEvent) => void>): (e: KeyboardEvent) => void {
  type Bind = {
    ctrl: boolean,
    alt: boolean,
    shift: boolean,
  ...
  return (e) => {
    ...
    for (const [chord, call] of valid) {
      const bind = chord[0];
      if (e.ctrlKey === bind.ctrl && e.shiftKey === bind.shift && e.altKey === bind.alt && e.key === bind.key) {
        if (chord.length === 1) {
          e.stopPropagation();
          call(e);
        } else {
