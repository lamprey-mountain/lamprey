...
const schema = new Schema({
  nodes: {
    ...
    mention: {
      ...
      parseDOM: [{ tag: "span[data-mention]", getAttrs: el => ({ user: el.dataset.mention }) }],
    },
    text: {
      group: "inline",
      inline: true,
    ...
    const view = new EditorView({ mount: editorEl }, {
      ...
      state: EditorState.create({
        ...
        plugins: [
          ...
          keymap({
            ...
            "Ctrl-i": createWrap("*"),
            "Ctrl-`": createWrap("`"),
            "Ctrl-m": (state) => {console.log(state.doc);return false},
        ...
        function walk(ast: Token) {
          switch (ast.type) {
            ...
            case "strong": {
              ...
              decorations.push(Decoration.inline(pos + 3, end - 2, { nodeName: "strong" }));
            ...
            case "link": {
              if (ast.raw === ast.href) {
                ...
                decorations.push(Decoration.inline(pos + 1, pos + hrefLen + 1, { class: "link" }));
              } else {
            ...
            case "code": {
              ...
              // syntax highlighting?
              decorations.push(Decoration.inline(pos + 1, pos + synLen + 1, { class: "syn" }));
              decorations.push(Decoration.inline(pos + synLen + codeLen + 2, end - synEndLen, { nodeName: "pre" }));
              decorations.push(Decoration.inline(end - synEndLen, end, { class: "syn" }));
              break;
            }
            case "blockquote": {
      ...
      handlePaste(view, event, slice) {
        ...
        if (/^(https?:\/\/|mailto:)\S+$/i.test(str) && !tr.selection.empty) {
          ...
          // console.log(slice)
          // slice.content.
          view.dispatch(tr.replaceSelection(slice).scrollIntoView().setMeta("paste", true).setMeta("uiEvent", "paste"));
      ...
      transformPastedHTML(html) {
        ...
        function walk(node, preserveSpace = true) {
          ...
          // DISABLED FOR NOW
          node.childNodes.forEach(el => walk(el, node.nodeName === "PRE" ? true : preserveSpace));
