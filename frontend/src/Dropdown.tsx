import { createEffect, createSignal, createUniqueId, For, Show, JSX, VoidProps } from "solid-js";
...
function createSelect<T>() {
  ...
  const [getFiltered, setFiltered] = createSignal<Array<Fuzzysort.KeyResult<DropdownItem<T>>>>([]);
  const [getFilter, setFilter] = createSignal("");
  const [getHovered, setHovered] = createSignal<DropdownItem<T>>();

  createEffect(() => {
    setFiltered(go(getFilter(), getItems(), { key: "label", all: true }) as unknown as Array<Fuzzysort.KeyResult<DropdownItem<T>>>);
  ...
  return {
    ...
    prev() {
      const idx = getFiltered().findIndex(i => i.obj === getHovered()!);
      setHovered(getFiltered()[(getFiltered().length + idx - 1) % getFiltered().length].obj);
    },
  };
}

export function Dropdown<T>(props: VoidProps<{
  selected?: T,
  required?: boolean,
  ...
  const [selected, setSelected] = createSignal<T | null>(props.selected ?? props.options[0]?.item ?? null);
