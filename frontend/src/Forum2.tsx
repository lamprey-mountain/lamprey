...
const InputReply = (props: { thread: Channel; reply: Message }) => {
	...
	const getName = (user_id: string) => {
		...
		const m = member();
		return (
			((m as any)?.membership === "Join" && (m as any)?.override_name) ??
			user()?.name
		);
	};
...
const Comment = (props: {
	...
	const canEditMessage = () => {
		return (
			(message() as any).type === "DefaultMarkdown" &&
			!message().is_local &&
			isOwnMessage()
		);
	};
			...
			<div class="inner">
				...
				<Show when={!collapsed()}>
					...
					>
						<Markdown
							content={
								(message().latest_version as any).type === "DefaultMarkdown"
					...
					<div style="display: flex">
						...
						<menu>
							<Switch>
								<Match
									when={
										(props.att as any).status === "uploading" &&
