...
impl Unnamed {
    ...
    pub async fn connect(mut self) -> Result<()> {
        ...
        let handle_ = Handle { globals: self.globals.clone() };
        let mut c = Client::new(token.clone().into()).with_handler(Box::new(handle_));
        
        tokio::spawn(async move {
            while let Some(msg) = self.recv.recv().await {
                let _ = self.handle(msg, &token).await;
            }
        });
    ...
    async fn handle(&self, msg: UnnamedMessage, token: &str) -> Result<()> {
        match msg {
