...
Deno.test("Audit Log Handling", async (t) => {
	...
	await t.step("Check room audit log", async () => {
		...
		const roomUpdateEntry = logs.audit_log_entries.find((entry: any) =>
			entry.type === "RoomUpdate"
		);
	...
	await t.step("Check user audit log", async () => {
		...
		const userUpdateEntry = logs.audit_log_entries.find((entry: any) =>
			entry.type === "UserUpdate"
		);
