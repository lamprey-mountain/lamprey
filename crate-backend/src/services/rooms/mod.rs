...
pub struct ServiceRooms2 {
    state: Arc<ServerStateInner>,

    /// deduplicate room create requests
    idempotency_keys: Cache<String, RoomHandle>,

    handler: Arc<DashMap<RoomId, Arc<RoomHandle>>>,
}

pub struct RoomHandle2 {
    // ...
}

pub struct RoomActor2 {
    state: Arc<ServerStateInner>,
    // room_id: RoomId,
...
impl ServiceRooms2 {
    ...
    async fn load(&self, room_id: RoomId) -> Arc<RoomHandle2> {
        todo!()
    }
}

impl RoomHandle2 {
    pub async fn snapshot(&self) -> Result<Arc<RoomSnapshot>> {
        todo!()
    }

    pub async fn ensure_members(&self) -> Result<()> {
        todo!()
    }

    pub async fn handle_sync(&self) -> Result<()> {
        todo!()
    }

    /// cleanup idle member lists
    pub async fn member_list_cleanup(&self) -> Result<()> {
        todo!()
    }

    /// subscribe to a new member list
    pub async fn member_list_subscribe(&self) -> Result<()> {
        todo!()
    }

    /// handle a member list command
    pub async fn member_list_command(&self) -> Result<Option<MessageSync>> {
        todo!()
    }
}

impl RoomActor2 {
    pub async fn load_members(&self) -> Result<RoomHandle> {
        todo!()
    }

    pub fn handle_sync(&self, sync: MessageSync) -> Result<()> {
        todo!()
    }
}
