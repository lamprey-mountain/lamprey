...
pub mod connection_queue; // TODO: rename to queue
pub mod error;
...
// TEMP: old sync connection code for reference, will be cleaned up soon
#[cfg(any())]
mod old {
    pub struct Connection {
        ...
        // TEMP: make this pub so that sync can poll it
        // i cant add fn poll_subscriptions(&mut self) since that would result in double mutable borrows...
        pub(crate) subscriptions: Box<ConnectionSubscriptions>,
    }
    ...
    impl Connection {
        ...
        pub async fn disconnect(&mut self) {
            // NOTE: should i auto resubscribe when the user reconnects/resumes? or should i force them to send another subscription message?
            ...
            match msg {
                ...
                MessageClient::Presence { presence } => {
                    let session = match &self.state {
                        ConnectionState::Unauthed => {
                            return Err(SyncErrorCode::Unauthenticated.into());
                        }
            ...
            if should_send {
                ...
                let msg = match *msg {
                    ...
                    MessageSync::VoiceState {
                        ...
                        let is_ours =
                            self.state.session().and_then(|s| s.user_id()) == Some(user_id);
                        if !is_ours {
