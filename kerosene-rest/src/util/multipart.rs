...
use crate::prelude::*;
use crate::util::error::{ExtractorError, ExtractorRejection};
use crate::util::parse::{parse_json, parse_msgpack};
use lamprey_backend_services::compat::routes::util::multipart::MultipartFile;
...
impl MultipartCollector {
    pub async fn collect(mut multipart: Multipart<'_>) -> CoreResult<Self, ExtractorRejection> {
        ...
        while let Some(field) = multipart
            .next_field()
            .await
            // NOTE: probably not an http error?
            .map_err(|e| ExtractorRejection::ServerError(ServerError::Internal(Box::new(e))))?
        {
    ...
    pub fn parse<T: DeserializeOwned>(self) -> CoreResult<(T, MultipartFiles), ExtractorRejection> {
        ...
        let body = serde_json::from_value(body)
            .map_err(|err| ExtractorRejection::ServerError(ServerError::Internal(Box::new(err))))?;

        Ok((body, self.media))
    }

    pub async fn into_files(self) -> MultipartFiles {
        self.media
    }

    async fn handle(&mut self, field: multer::Field<'_>) -> CoreResult<(), ExtractorRejection> {
        let Ok(name) = field
            .name()
