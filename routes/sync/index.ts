...
export default function setup(app: OpenAPIHono<HonoEnv>) {
  app.openapi(SyncInit, async (c, next) => {
    ...
    function send(msg: z.infer<typeof MessageServer>) {
      if (state === "closed") throw new Error("tried to send message to closed websocket");
      ...
      onMessage(event, ws) {
        try {
          ...
          if (msg.type === "hello") {
            ...
            const rowSession = db.prepareQuery("SELECT * FROM sessions WHERE token = ?").firstEntry([msg.token]);
            if (!rowSession) return c.json({ error: "Invalid or expired token" }, 401);
            // if (row.level as number < 1) return c.json({ error: "Unauthorized" }, 403);
            user_id = rowSession.user_id as string;
            const rowUser = db.prepareQuery("SELECT * FROM users WHERE user_id = ?").firstEntry([user_id]);
            if (!rowUser) throw new Error("user doesn't exist, but session does...!?");
            state = "auth";
            send({
              type: "ready",
