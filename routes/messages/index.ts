...
import { MessageCreate, MessageDelete, MessageList, MessageUpdate, MessageVersionsDelete, MessageVersionsGet, MessageVersionsList } from "./def.ts";
import { withAuth } from "../auth.ts";
import { uuidv7 } from "uuidv7";
import { MessageFromDb } from "../../types/db.ts";
import { UUID_MIN, UUID_MAX } from "../../util.ts";
...
export default function setup(app: OpenAPIHono<HonoEnv>) {
  app.openapi(withAuth(MessageCreate), async (c) => {
    ...
    const message = Message.parse({ ...MessageFromDb.parse({ ...row, room_id }), nonce: r.nonce });
  ...
  app.openapi(withAuth(MessageList), (c) => {
    ...
    return c.json({
      ...
      total: count,
      messages: rows.slice(0, limit).map(i => Message.parse(MessageFromDb.parse({ ...i, room_id }))),
    });
  });
