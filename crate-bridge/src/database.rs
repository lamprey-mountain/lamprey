...
pub trait Database: fmt::Debug + Send + Sync {
    ...
    async fn portal_get_by_discord_channel(
        &self,
        discord_channel_id: String,
    ) -> Result<Option<Portal>>;
    async fn portal_get_by_lamprey_channel(
        &self,
    ...
    async fn message_get_by_discord_id(
        &self,
        portal_id: PortalId,
        discord_message_id: discord::MessageId,
    ) -> Result<Option<Message>>;
...
impl Database for SqliteDatabase {
    ...
    async fn realm_delete(&self, id: RealmId) -> Result<()> {
        ...
        query!("DELETE FROM realm WHERE id = ?", id_str)
            .execute(&self.pool)
            .await?;
        Ok(())
    }
    ...
    async fn portal_create(&self, portal: Portal) -> Result<()> {
        ...
        let portal_id = portal.id.to_string();
        let realm_id_str = portal.realm_id.map(|id| id.to_string());
    ...
    async fn portal_list(&self) -> Result<Vec<Portal>> {
        ...
        Ok(rows
            .into_iter()
            .map(|r| Portal {
                id: r.id.expect("portal id").parse::<Uuid>().unwrap().into(),
                realm_id: r.realm_id.map(|id| id.parse::<Uuid>().unwrap().into()),
    ...
    async fn message_get_by_discord_id(
        ...
        let portal_id_str = portal_id.to_string();
        let row = query!(
            "SELECT id, source_platform, lamprey_message_id, discord_message_id FROM message WHERE portal_id = ? AND discord_message_id = ?",
            portal_id_str,
            discord_message_id_str
        )
        .fetch_optional(&self.pool)
