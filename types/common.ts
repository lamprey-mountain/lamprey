...
export const RoomId           = z.string().uuid().describe("A unique identifier for this Room").openapi({ title: "RoomId", example: roomIdExample });
export const UserId           = z.string().uuid().describe("A unique identifier for this User").openapi({ title: "UserId", example: userIdExample });
export const ThreadId         = z.string().uuid().describe("A unique identifier for this Thread").openapi({ title: "ThreadId", example: threadIdExample });
export const MessageId        = z.string().uuid().describe("A unique identifier for this Message").openapi({ title: "MessageId", example: messageIdExample });
export const MessageVersionId = z.string().uuid().describe("A unique identifier for this MessageVersion").openapi({ title: "MessageVersionId", example: messageVersionIdExample });
export const MemberId         = z.string().uuid().describe("A unique identifier for this Member").openapi({ title: "MemberId", example: memberIdExample });
export const RoleId           = z.string().uuid().describe("A unique identifier for this Role").openapi({ title: "RoleId", example: roleIdExample });
export const SessionId        = z.string().uuid().describe("A unique identifier for this Session").openapi({ title: "SessionId", example: sessionIdExample });
export const MediaId          = z.string().uuid().describe("A unique identifier for this Media").openapi({ title: "MediaId", example: mediaIdExample });
export const AuditLogEntryId  = z.string().uuid().describe("A unique identifier for this AuditLogEntry").openapi({ title: "AuditLogEntry", example: auditLogEntryIdExample });

export const SessionToken = z.string().describe("A secret token that authorizes this session").openapi({ title: "SessionToken", example: sessionTokenExample });
export const InviteCode = z.string().openapi({ title: "InviteCode", example: "arstdhneio" });
...
export const Media = z.object({
  ...
  source_url: z.string().optional().describe("The source url this media was downloaded from, if any"),
  thumbnail_url: z.string().optional().describe("A thumbnail"),
  mime: z.string().describe("The mime type (file type)"),
  alt: z.string().optional().describe("Descriptive alt text, not entirely unlike a caption"),
...
export const Room = z.object({
  ...
  example: {
    ...
    description: "i expect i'll be able to solve a lot of my problems once my baby brain falls out and my adult brain grows in",
  }
});
...
export const InvitePatch = Invite.pick({ expires_at: true, max_uses: true }).partial();
export const MemberPatch = Member.pick({ override_name: true, override_description: true, roles: true }).partial();
