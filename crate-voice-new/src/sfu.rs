...
use common::{
    v1::types::voice::{
        ...
        KeyframeRequestKind, Mid, Rid, Speaking, SpeakingWithUserId, TrackMetadata,
...
impl Shard {
    ...
    fn route_media(&mut self, publisher_id: PeerId, media: str0m::media::MediaData) {
        let Some(track_id) = self.peers[publisher_id].lookup_track(media.mid) else {
            return;
        };

        let always_sub = self.tracks[track_id].is_always_subscribed();
        ...
        for target_pid in peer_ids {
            ...
            let peer = &mut self.peers[target_pid];

            // TODO: handle rid/simulcast
            if always_sub || peer.subscriptions.tracks.contains(&track_id) {
                peer.write_media(track_id, &media);
            }
        }
    }
    ...
    async fn handle_peer_event(&mut self, peer_id: PeerId, event: Event) {
        match event {
            Event::Connected => {
                ...
                let peer_type: String = todo!("get as User(uuid) or Sfu(uuid)");
                debug!(
                    %peer_type,
                    channel_id = ?self.channel_id,
                    "Peer connected",
