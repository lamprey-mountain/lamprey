...
impl DataThread for Postgres {
    ...
    async fn thread_all_active_room(&mut self, room_id: RoomId) -> Result<Vec<Channel>> {
        ...
        let items = query_as!(
            ...
            r#"
            SELECT
                c.id, c.room_id, c.creator_id, c.owner_id, c.version_id, c.name, c.description, c.icon, c.url, c.type as "ty: _", c.nsfw, c.latest_seq, c.locked, c.locked_until, c.locked_roles, c.archived_at, c.deleted_at, c.parent_id, c.position, c.bitrate, c.user_limit, c.invitable, c.auto_archive_duration,
                c.default_auto_archive_duration, c.slowmode_thread, c.slowmode_message,
                c.default_slowmode_message, c.last_activity_at, c.last_pin_timestamp,
                (SELECT coalesce(COUNT(*), 0) FROM thread_member WHERE channel_id = c.id AND membership = 'Join') AS "member_count!",
                c.last_version_id as last_version_id,
                ...
                (SELECT coalesce(COUNT(*), 0) FROM channel_tag WHERE channel_id = c.id) AS "tag_count!",
                (SELECT row_to_json(cd.*) FROM channel_document cd WHERE cd.channel_id = c.id) as document,
                (SELECT row_to_json(cw.*) FROM channel_wiki cw WHERE cw.channel_id = c.id) as wiki,
                (SELECT row_to_json(cc.*) FROM channel_calendar cc WHERE cc.channel_id = c.id) as calendar
            FROM channel c
            WHERE
                c.room_id = $1
