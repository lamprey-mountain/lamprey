import type { Lamprey } from "../client";
import type { schemas } from "../schema";
import { Manager } from "./common";

export type ApiRoom = schemas["Room"];
...
export class RoomManager extends Manager<Room, string> {
	...
	async fetch(id: string): Promise<Room> {
		// const b = this.client.options.backend;
		// if (b === "direct") {
		// 	await fetch("...");
		// } else if (b === "sharedWorker") {
		// 	const worker: SharedWorker;
		// 	worker.port.postMessage({ type: "fetch room..." });
		// }
