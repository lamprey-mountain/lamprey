...
export type SyncerEvents = {
	sync(sync: schemas["MessageEnvelope"]): void;
	error(error: Error): void;
	ready(ready: schemas["MessageReady"]): void;
	state(state: SyncerState): void;
};
...
export class Syncer extends Emitter<SyncerEvents> {
	...
	private sendQueue: Array<schemas["MessageClient"]> = [];
	...
	private handleMessage(msg: schemas["MessageEnvelope"]) {
		switch (msg.op) {
			case "Ping": {
	...
	private sendInner(m: schemas["MessageClient"], force = false) {
	...
	private connect() {
		...
		this.ws.addEventListener("message", (e) => {
			...
			let msg: schemas["MessageEnvelope"];
			if (isBinary) {
		...
		this.ws.addEventListener("open", (_e) => {
			...
			this.sendInner(
				// HACK: openapi thinks resume is required
				{
					...
					token: this.token,
					...this.resume,
				} as unknown as schemas["MessageClient"],
				true,
			);
		});
	...
	public send(message: schemas["MessageClient"]) {
...
function createDecompressor(
	format: "json" | "msgpack",
	onMessage: (msg: schemas["MessageEnvelope"]) => void,
	...
	(async () => {
		try {
			while (true) {
				...
				if (done) break;
				if (value) onMessage(value as schemas["MessageEnvelope"]);
			}
		} catch (err) {
